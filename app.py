...
Kisan Helpline: 1800-180-1551"""
        ...
        # ── VOICE OUTPUT FOR CHATBOT REPLY ──
        st.session_state["last_bot_audio"] = None
        if enable_voice:
            try:
                st.session_state["last_bot_audio"] = text_to_speech(bot_ans, lang_code)
            except Exception:
                st.session_state["last_bot_audio"] = None
    ...
    # ── PLAY VOICE OUTPUT FOR THE LATEST REPLY ──
    if active_query and enable_voice and st.session_state.get("last_bot_audio"):
        st.markdown("#### Marathi/Hindi Voice Advisory (आवाज उत्तर)")
        st.audio(st.session_state["last_bot_audio"], format="audio/mp3")
